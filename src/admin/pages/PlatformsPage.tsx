...
const PlatformsPage: React.FC = () => {
  ...
  const filteredPlatforms = platforms.filter(platform =>
    ...
    platform.slug.toLowerCase().includes(search.toLowerCase())
  );
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingPlatform) {
        ...
        await adminApi.createPlatform(formData);
      ...
      alert(err?.response?.data?.error?.message || 'Failed to save platform');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      alert(err?.response?.data?.error?.message || 'Failed to delete platform');
    } finally {
      setDeleting(false);
    }
  };
      ...
      <div style={{ 
        ...
        <div>
          ...
          <p style={{ color: theme.colors.textSecondary }}>
            {platforms.length} platforms total
          </p>
        </div>
        <button
                  ...
                  <td style={{ padding: '16px', textAlign: 'right' }}>
                    <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end' }}>
                      ...
                      <button
                        ...
                        disabled={platform.gamesCount > 0}
                        style={{
                          padding: '8px',
                          borderRadius: '6px',
                          border: 'none',
                          backgroundColor: platform.gamesCount > 0 ? theme.colors.surface : `${theme.colors.error}20`,
                          color: platform.gamesCount > 0 ? theme.colors.textSecondary : theme.colors.error,
                          cursor: platform.gamesCount > 0 ? 'not-allowed' : 'pointer',
                          opacity: platform.gamesCount > 0 ? 0.5 : 1,
                        }}
                        title={platform.gamesCount > 0 ? 'Cannot delete platform with games' : 'Delete'}
                      >
                        <FiTrash2 size={16} />
                      </button>
                    </div>
              ...
              {filteredPlatforms.length === 0 && (
                <tr>
                  <td colSpan={5} style={{ 
                    ...
                    {search ? 'No platforms found' : 'No platforms yet'}
                  </td>
                </tr>
              )}
            </tbody>
