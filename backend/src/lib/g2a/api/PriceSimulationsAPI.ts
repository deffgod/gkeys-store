...
export class PriceSimulationsAPI {
  ...
  async simulate(
    ...
    return this.executeRequest('/prices/simulations', 'PriceSimulationsAPI.simulate', async () => {
      ...
      const response = await this.httpClient.get<PriceSimulationResponse>('/prices/simulations', {
        params,
      });
  ...
  async batchSimulate(
    ...
    Array<{ productId: string; simulation: PriceSimulationResponse | null; error?: Error }>
  > {
