...
export class ProductsAPI {
  ...
  async list(filters?: G2AProductFilters): Promise<G2AProductsResponse> {
    return this.executeRequest('/products', 'ProductsAPI.list', async () => {
      ...
      if (filters?.includeOutOfStock !== undefined) params.includeOutOfStock = filters.includeOutOfStock;
   ...
   */
  async search(query: string, filters?: Omit<G2AProductFilters, 'id'>): Promise<G2AProductsResponse> {
